...
- "text" は偽論文のタイトルとサブセクションの"title_name"を考慮した学術論文風の文章にしてください。文字数は長めに設定し、改行コードや段落を適切に含めてください。嘘の論文ですが、ユーモのある読んでて面白い内容にしてください。
...
- 例えば1つ目のサブセクションの"text"で"[INSERT_FORMULA_HOGEHOGE]"としたならば必ずそのサブセクション内のformulasプロパティにデータを設定してください。
- グラフ、表、数式を含める場合は"text"にてそのデータに言及するような内容を必ず含めてください。
- 嘘論文をそれっぽく見せるというコンセプトなのですがユーモアのある面白い内容に仕立てて作成してください。
