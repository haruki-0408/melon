...
from melon.layers.common.utilities import upload_to_s3
...
def lambda_handler(event, context):
    ...
    styles.add(ParagraphStyle(name="FakeThesisTitle", fontName="IPAexMincho", fontSize=24, alignment=1, spaceAfter=20))
    ...
    styles.add(ParagraphStyle(name="Author", fontName="IPAexMincho", fontSize=12, alignment=2, spaceAfter=20))
    ...
    elements.append(Paragraph(title, styles['FakeThesisTitle']))
    elements.append(Spacer(1, 20))
    elements.append(Paragraph(f"著者: {author}", styles['Author']))  # 著者名を右寄せで表示
    elements.append(Spacer(1, 60))
    elements.append(Paragraph("要旨", styles['JapaneseHeading2']))
